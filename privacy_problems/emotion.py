...
from consts import HAARCASCADE_FACE_MODEL, EMOTION_MODEL, EMOTION_TFLITE_MODEL


def web_cam(face_detector,model,src=0,vid_rec = True):
    """
    ...
    while 1:
        ...
        faces = faceCascade.detectMultiScale(
            ...
            minNeighbors=7,
            minSize=(100, 100),)
        print("face: --- %s seconds ---" % (time.time() - start_time))
            ...
            if len(faces) > 0:
                for x, y, width, height in faces:
                    ...
                    if frame_count % 10 == 0:
                        ...
                        print("emotion:--- %s seconds ---" % (time.time() - start_time))
...
def photo(faceCascade, model):
    ...
    for photo in os.listdir(photo_path):
        frame = cv2.imread(photo_path + photo)
...
def main():
    ...
    face_detector = cv2.CascadeClassifier(HAARCASCADE_FACE_MODEL.as_posix())
    emotion_model = load_model(EMOTION_MODEL.as_posix())
    web_cam(face_detector,emotion_model)
    # photo(face_detector, emotion_model)
